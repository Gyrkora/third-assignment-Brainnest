...
function uservali() {
	...
	if (user === "") {
		...
		uservali();
	     ...
	     let conf= confirm('Sorry but you pressed "Cancel!":( Press "Ok"to Enter name or Cancel to exit')
		 if(conf){
			uservali()
		 }else{
...
function replay(){
	let replayConfirm= confirm('Would You like to play again..Press "ok" to replay or "cancle" to quite')
	if (replayConfirm){
		while(replayConfirm){
			botWin=0
			playerWin=0
